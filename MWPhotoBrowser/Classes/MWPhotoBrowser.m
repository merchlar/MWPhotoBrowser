...
//    _toolbar.barStyle = UIBarStyleBlackTranslucent;
    ...
    _actionButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"MWPhotoBrowser.bundle/images/share.png"] style:UIBarButtonItemStylePlain target:self action:@selector(actionButtonPressed:)];
...
- (void)updateNavigation {
	...
	if ([self numberOfPhotos] > 1) {
		lbNavTitle.text = [NSString stringWithFormat:@"%i %@ %i", _currentPageIndex+1, NSLocalizedString(@"Gallery_Separator", @"Used in the context: 'Showing 1 of 3 items'"), [self numberOfPhotos]];
	} else {
		lbNavTitle.text = nil;
	}
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if ([MFMailComposeViewController canSendMail]) {
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        cancelButtonTitle:NSLocalizedString(@"Gallery_Cancel", nil) destructiveButtonTitle:nil
                                                        otherButtonTitles:NSLocalizedString(@"Gallery_Save", nil), NSLocalizedString(@"Gallery_Copy", nil), NSLocalizedString(@"Gallery_Email", nil), nil] autorelease];
            } else {
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        cancelButtonTitle:NSLocalizedString(@"Gallery_Cancel", nil) destructiveButtonTitle:nil
                                                        otherButtonTitles:NSLocalizedString(@"Gallery_Save", nil), NSLocalizedString(@"Gallery_Copy", nil), nil] autorelease];
            }
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    [self showProgressHUDCompleteMessage: error ? NSLocalizedString(@"Gallery_Save_Failed", @"Informing the user a process has failed") : NSLocalizedString(@"Gallery_Save_Saved", @"Informing the user an item has been saved")];
    [self hideControlsAfterDelay]; // Continue as normal...
}
...
- (void)actuallyCopyPhoto:(id<MWPhoto>)photo {
    if ([photo underlyingImage]) {
        ...
        [self showProgressHUDCompleteMessage:NSLocalizedString(@"Gallery_Copy_Copied", @"Informing the user an item has finished copying")];
        [self hideControlsAfterDelay]; // Continue as normal...
    }
}
...
- (void)actuallyEmailPhoto:(id<MWPhoto>)photo {
    if ([photo underlyingImage]) {
        ...
        [emailer setSubject:NSLocalizedString(@"Gallery_Email_Body", nil)];
...
- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
    if (result == MFMailComposeResultFailed) {
		UIAlertView *alert = [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Gallery_Email", nil)
                                                         message:NSLocalizedString(@"Gallery_Email_Fail", nil)
                                                        delegate:nil cancelButtonTitle:NSLocalizedString(@"Gallery_Alert_Dismiss", nil) otherButtonTitles:nil] autorelease];
		[alert show];
    }
	[self dismissModalViewControllerAnimated:YES];
}
